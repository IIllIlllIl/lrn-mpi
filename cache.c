...
/* "int"s per data cache for Apple Silicon M1 (64KB) */
#define cache 64*1024
...
int main (int argc, char *argv[])
{
   ...
   block = (cache / sizeof(int)) * 0.9;
   
   // prime in 0 ~ sqrt(n)
   visit = (int *) malloc ((int) sqrt((double) n) * sizeof(int));
   prm = (int *) malloc ((int) sqrt((double) n) * sizeof(int));

   for (i = 0; i < (int) sqrt((double) n); i++) visit[i] = 0;
   for (i = 0; i < (int) sqrt((double) n); i++) prm[i] = 0;
   ...
   for(low = low_value; low < high_value; low += block){
      high = MIN(low + block - 1, high_value);

      /* Allocate this process's share of the array. */
      marked = (char *) malloc (high - low);
      // memset
      for (i = 0; i < (high - low); i++) marked[i] = 0;
